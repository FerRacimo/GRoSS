...
- 2a) a graph file describing the topology of the graph, specified with the -r option. This can be in the same format as the graph file that is used as input for qpGraph (Patterson et al. 2012). Note that the value of the fitted admixture weights must be correctly specified.
- 2b) alternatively to 2.a), one can instead specify a dotfile (outputted from qpGraph after fitting) with the -d option 
